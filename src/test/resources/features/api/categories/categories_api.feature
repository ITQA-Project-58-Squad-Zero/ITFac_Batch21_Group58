@api
Feature: Category Management API - Read Operations
  As an Admin or User
  I want to access category data via API
  So that I can verify category endpoints and data

  @admin_auth @CM1_API_001
  Scenario: CM1_API_001 Get All Categories
  ...
  @admin_auth @CM1_API_002
  Scenario: CM1_API_002 Get Category by ID (Success)
    Given the admin has a valid session
  ...
  @admin_auth @CM1_API_003
  Scenario: CM1_API_003 Get Main Categories Only
    Given the admin has a valid session
    When I request main categories
    Then the response status code should be 200
    And each returned category should have no parent

  @admin_auth @CM1_API_004
  Scenario: CM1_API_004 Get Category Summary
    Given the admin has a valid session
